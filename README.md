...
| 💌 Shall We Talk 三件套 | 三套话术（直球 / 温柔 / 半开玩笑）+ 模拟演习 + 事后预案 |
...
平台支持技能机制时（如 OpenClaw / Claude Code），把整个 `shall-we-talk/` 文件夹复制到技能目录：

```bash
# OpenClaw 示例
cp -r shall-we-talk ~/.openclaw/workspace/skills/shall-we-talk
openclaw skills list   # 看到 shall-we-talk 即安装成功
```
...
> 💡 传输 / 下载后文件名若带 `---` 暂存前缀（如 `SKILL---xxx.md`），重命名为标准名即可：`SKILL.md`、`README.md`、`templates/heart-report.md`、`templates/shall-we-talk-scripts.md`。
...
或在支持 `/skill` 命令的平台（如 OpenClaw）：`/skill shall-we-talk`
...
模板在 OpenClaw 中通过 `{baseDir}` 引用（指技能根目录 `shall-we-talk/`），例如 `{baseDir}/templates/heart-report.md`；其他平台不支持 `{baseDir}` 时，把模板内容内联进 SKILL.md 或按相对路径引用即可。
