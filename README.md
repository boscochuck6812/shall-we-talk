...
一个教人「把话说开」的 AgentSkill：从聊天记录分析感情状态，并指导用户完成「确定关系」的关键对话。

## 这是什么

现有恋爱类 AI 技能都在「分析对方心意」（测信号、测暧昧），但没有一个教用户「开口推进关系」。本技能填补空白：

1. 分析聊天记录，输出六维「心动体检报告」；
2. 定制「我们是什么关系」摊牌话术（三套风格任选）；
3. 模拟演习对方三种回答的接话预案；
4. 提供被拒 / 成功之后的事后预案。
...
| 护栏 | 区分偶发波动与系统性问题、禁止制造焦虑、PUA 风险提示、拒绝操控类请求 |
...
或在支持 `/skill` 命令的平台（如 OpenClaw）手动指定：`/skill the-talk`；其他 AI 直接说触发词即可。

## 安装

本技能是纯文本 SKILL.md，**不绑定任何平台**——Claude / GPT / Gemini / DeepSeek / OpenClaw 等任意 AI 通用。

**用法一：作为技能文件安装**（平台支持技能机制时）

1. 把整个 `the-talk/` 文件夹复制到技能目录，以 OpenClaw 为例：

   ```
   ~/.openclaw/workspace/skills/the-talk/
   ```
...
2. 验证（OpenClaw 为例）：
   ...
   看到 `the-talk` 即安装成功；其他平台按各自约定放置并确认加载。

**用法二：直接粘贴使用**（任何 AI 通用）

把 `SKILL.md` 全文（含 frontmatter，模板内容一并附上）粘贴给任意 AI，说一句「我们该不该摊牌」即可开始。

> 提示：传输 / 下载后文件名若带 `---` 暂存前缀（如 `SKILL---xxx.md`），重命名为标准名即可：`SKILL.md`、`README.md`、`templates/heart-report.md`、`templates/the-talk-scripts.md`。
...
模板在 OpenClaw 中通过 `{baseDir}` 引用（`{baseDir}` 指技能根目录，即 `the-talk/`），例如 `{baseDir}/templates/heart-report.md`；其他平台不支持 `{baseDir}` 时，把模板内容内联进 SKILL.md 或按相对路径引用即可。
